...
\begin{document}
    ...
    Por el contrario, el escenario con presencia de fuente (línea naranja) exhibe una discontinuidad manifestada justo en el entorno del centro geométrico ($x=0.5$), proyectando un incremento abrupto y empinado de temperatura que roza los $102^\circ\text{C}$. Este pico local es un claro indicador de la influencia dominante de la fuente puntual, superando incluso el valor máximo impuesto por el borde superior. La forma asimétrica y pronunciada del perfil en este caso refleja la naturaleza no homogénea de la ecuación o la alta localización espacial de la respuesta térmica ante perturbaciones focalizadas, evidenciando un fenómeno de saturación térmica que se extiende hacia los bordes adyacentes.
